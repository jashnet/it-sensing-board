...
def load_user_settings(user_id):
    ...
    if os.path.exists(filename):
        ...
        settings = {
            ...
            "channels": get_initial_channels() # 외부 DB 로직 핵심
        }
...
# --- 5. 사이드바 제어 (수정 사항 집중 반영) ---
with st.sidebar:
    st.title("👤 User Profile")
    # 사용자 선택 기능
    selected_user = st.radio("사용자 선택", ["1", "2", "3", "4"], horizontal=True)
    
    # 사용자 변경 감지 시 설정 로드
    if "current_user" not in st.session_state or st.session_state.current_user != selected_user:
        st.session_state.current_user = selected_user
        st.session_state.settings = load_user_settings(selected_user)
        if "news_data" in st.session_state: del st.session_state.news_data # 데이터 초기화
        st.rerun()
    ...
    if st.session_state.settings["api_key"] and not st.session_state.show_api:
        st.success(f"✅ User {selected_user} 연동됨")
        ...
        if st.button("저장 및 적용"):
            ...
            save_user_settings(selected_user, st.session_state.settings); st.rerun()
        ...
        if st.session_state.settings["category_active"][cat]:
            with st.expander(f"{cat} 관리"):
                ...
                for idx, f in enumerate(st.session_state.settings["channels"][cat]):
                    ...
                    f["active"] = col_ch.checkbox(f["name"], value=f["active"], key=f"side_{selected_user}_{cat}_{idx}")
                    if col_del.button("❌", key=f"del_{selected_user}_{cat}_{idx}"):
                        st.session_state.settings["channels"][cat].pop(idx)
                        save_user_settings(selected_user, st.session_state.settings); st.rerun()
                
                st.markdown("---")
                # 카테고리별 채널 추가 기능
                st.caption(f"➕ {cat} 채널 추가")
                with st.form(f"add_form_{cat}_{selected_user}", clear_on_submit=True):
                    new_ch_name = st.text_input("채널 이름")
                    new_ch_url = st.text_input("RSS URL")
                    if st.form_submit_button("저장"):
                        if new_ch_name and new_ch_url:
                            st.session_state.settings["channels"][cat].append({"name": new_ch_name, "url": new_ch_url, "active": True})
                            save_user_settings(selected_user, st.session_state.settings); st.rerun()
    ...
    if st.button("🚀 Apply & Sensing Start", use_container_width=True):
        save_user_settings(selected_user, st.session_state.settings)
        if "news_data" in st.session_state: del st.session_state.news_data
        st.rerun()

# --- 6. 모던 UI 디자인 및 메인 렌더링 ---
...
if raw_data:
    ...
    st.subheader(f"🌟 Strategic Top Picks (User {selected_user})")
    top_6 = raw_data[:6]
