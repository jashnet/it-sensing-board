...
def get_initial_channels():
    return {
        "Global Innovation": [
            ...
            {"name": "Threads-Tech", "url": "https://rss.itdog.icu/threads/user/zuck", "active": True}
            # ... 추가 100여개 이상의 글로벌 채널을 포함한 200개 규모
        ],
        "China & East Asia": [
...
def load_user_settings(user_id):
    ...
    default_settings = {
        ...
        "sensing_period": 7, "max_articles": 30, "filter_weight": 70, "filter_strength": 3,
        "filter_prompt": "제조사별 차세대 디바이스, 혁신적 하드웨어 디자인 소식 위주.",
        "ai_prompt": "삼성전자 CX 기획자 관점 분석: 1.요약 2.영향 3.제언",
...
# --- 6. 모던 화이트 UI 설정 ---
st.set_page_config(page_title="NGEPT Hub v11.8", layout="wide")
st.markdown("""
<style>
...
with st.sidebar:
    ...
    for cat in list(st.session_state.settings["channels"].keys()):
        ...
        active_count = len([c for c in ch_list if c["active"]])
        st.session_state.settings["category_active"][cat] = st.toggle(f"{cat} ({active_count}/{len(ch_list)})", value=st.session_state.settings["category_active"].get(cat, True))
    ...
    with st.expander("⚙️ 고급 전략 설정", expanded=True):
        ...
        st.session_state.settings["filter_prompt"] = st.text_area("필터 프롬프트", value=st.session_state.settings["filter_prompt"], height=100)
